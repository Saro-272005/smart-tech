...
// CORRECTED: isToday logic to correctly compare Date objects (for dashboard only)
...
// CORRECTED: Use moment.js style YYYY-MM-DD comparison directly on the strings
function matchesDate(orderDateString, targetDateString) {
    if (!targetDateString) return true;

    // Ensure database date string is formatted consistently (YYYY-MM-DD)
    const orderDate = new Date(orderDateString);
    const year = orderDate.getFullYear();
    // Months are 0-indexed, so we add 1, and pad with 0 if single digit
    const month = String(orderDate.getMonth() + 1).padStart(2, '0'); 
    const day = String(orderDate.getDate()).padStart(2, '0');
    const orderYMD = `${year}-${month}-${day}`; 
    
    // The targetDateString from the input field is already in YYYY-MM-DD format
...
let isPaid = false;
// MODIFIED LOGIC: Check for 'Paid' status based on server logic (server.js)
if (paymentMethod === 'Online Pay' || paymentMethod === 'COD - Paid' || paymentMethod === 'online' || paymentMethod === 'upi') {
isPaid = true;
} 
return {
...
// NEW HELPER: Get the user-friendly payment method name
function getPaymentMethodDisplay(paymentMethod) {
    if (!paymentMethod) return 'N/A';
    const lowerCaseMethod = paymentMethod.toLowerCase();
    if (lowerCaseMethod === 'cod' || lowerCaseMethod === 'cod - paid') {
        return 'Cash on Delivery';
    }
    if (lowerCaseMethod === 'online pay' || lowerCaseMethod === 'online') {
        return 'Online Payment';
    }
    return paymentMethod.toUpperCase().replace('-', ' - ');
}
...
// console.log("🟢 Starting to load dashboard stats..."); // HIDDEN
$$('.stat-value').forEach(el => el.textContent = '...');
try {
const ordersRes = await fetch("http://localhost:5000/api/orders");
if (!ordersRes.ok) throw new Error("Failed to fetch orders.");
const allOrders = await ordersRes.json();
// console.log("✅ All orders fetched:", allOrders.length); // HIDDEN
...
renderDailyRevenueChart(allOrdersDataForDashboard, selectedYear, selectedMonth);
}
};
...
// NEW HELPER: Get the products for the 'New Arrivals' section on the main home page
function getHomePageNewArrivals() {
    // Sort all products by ID (assuming higher ID means newer) and take the first 4
    const newProducts = allProductsData.slice().sort((a, b) => b.id - a.id).slice(0, 4);
    
    // Your specific requirement: one mobile, one laptop, one TV, one accessory
    // This is hard to guarantee with generic sorting, so we'll filter from the newest 4 to satisfy the category diversity, prioritizing the first 4 newest.
    
    let mobile = newProducts.find(p => p.category === 'mobiles');
    let laptop = newProducts.find(p => p.category === 'laptops');
    let tv = newProducts.find(p => p.category === 'televisions');
    let accessory = newProducts.find(p => ['accessories', 'smartwatches', 'headphones'].includes(p.category));

    // Fallback if not all categories are in the newest 4
    if (!mobile) mobile = allProductsData.find(p => p.category === 'mobiles');
    if (!laptop) laptop = allProductsData.find(p => p.category === 'laptops');
    if (!tv) tv = allProductsData.find(p => p.category === 'televisions');
    if (!accessory) accessory = allProductsData.find(p => ['accessories', 'smartwatches', 'headphones'].includes(p.category));

    // Ensure only unique, non-null items are returned, limiting to 4
    const finalNewArrivals = [mobile, laptop, tv, accessory].filter(p => p).reduce((unique, item) => {
        return unique.find(p => p.id === item.id) ? unique : [...unique, item];
    }, []).slice(0, 4);

    return finalNewArrivals;
}

// NEW HELPER: Get the products for the 'Budget Essentials' section on the main home page
function getHomePageBudgetEssentials() {
    // Filter for smartwatches and accessories with an effective price under ₹1000
    const budgetItems = allProductsData.filter(p => {
        const isBudgetCategory = p.category === 'smartwatches' || p.category === 'accessories';
        
        // Determine the lowest effective price (regular price or variant price)
        let effectivePrice = parseFloat(p.price || p.mrp_price || Infinity);

        if (p.variants && p.variants.length > 0) {
             const variantPrices = p.variants.map(v => parseFloat(v.price || v.mrp_price || Infinity));
             effectivePrice = Math.min(...variantPrices);
        }

        return isBudgetCategory && effectivePrice < 1000 && effectivePrice > 0;
    });

    // Take only the first 3 images/products
    return budgetItems.slice(0, 3);
}
...
const isPrepaid = orderToUpdate.payment_method.toLowerCase().includes('online') || orderToUpdate.payment_method.toLowerCase().includes('upi');
if (isPrepaid) {
...
// --- START NEW HTML STRUCTURE (MODIFIED FOR CORRECT PRICE DISPLAY) ---
const productsHtml = Array.isArray(products) ? products.map(p => {
    ...
    // Use MRP price for Unit Price display. Ensure it's parsed as float.
    const unitMRP = parseFloat(p.mrp_price || 0); // Use mrp_price for MRP display
    const formattedUnitMRP = formatCurrencyINR(unitMRP);
    
    // Calculate Item Total: Use discounted price 'price' if available and valid (>0), otherwise fallback to 'mrp_price'.
    // Ensure both are parsed as float
    let effectiveUnitPrice = parseFloat(p.price || 0);
    if (effectiveUnitPrice <= 0) {
         effectiveUnitPrice = parseFloat(p.mrp_price || 0);
    }
    
    const itemQuantity = parseInt(p.quantity || 1);
    const itemTotal = effectiveUnitPrice * itemQuantity;
    const formattedItemTotal = formatCurrencyINR(itemTotal);
    ...
    return `
        <div class="modal-product-item" style="border: 1px solid #ddd; padding: 10px; margin-bottom: 10px; border-radius: 4px; display: flex; align-items: flex-start; gap: 10px;">
            ...
            <div style="flex-grow: 1;">
                <p style="margin: 0 0 5px 0;"><strong>Product:</strong> ${productName}${variant} (Qty: ${itemQuantity})</p>
                <p style="margin: 0; font-size: 0.9em; color: #555;"><strong>MRP Price (Unit):</strong> ${formattedUnitMRP}</p>
                <p style="margin: 0; font-weight: bold; color: #2ecc71;"><strong>Item Total:</strong> ${formattedItemTotal}</p>
            </div>
        </div>
    `;
}).join('') : '<p>No product summary available.</p>';

// MODIFIED: Use the new helper function for display
...
function addVariantRow(specName = '', price = '', mrp_price = '', discountEndDate = '', stock = '', isFirst = false) {
    ...
    // Check the current category to determine if inputs should be required
    const currentCategory = $('#pCategory')?.value || 'mobiles';
    const usesVariants = ['mobiles', 'seconds', 'laptops'].includes(currentCategory);
    ...
    // Set 'required' attribute dynamically based on current category
    // This is the key change to prevent validation errors when variants are hidden
    const requiredAttr = usesVariants ? 'required' : '';

    variantGroup.innerHTML = `
        ${removeButtonHTML}
        <div class="form-field">
            <label for="variant-name">Specification</label>
            <input type="text" class="variant-name" name="variant-name" placeholder="" value="${specName}" ${requiredAttr}/>
        </div>
        <div class="variant-price-group">
            <div class="form-field">
                <label for="variant-mrp-price">MRP Price</label>
                <input type="number" class="variant-mrp-price" name="variant-mrp-price" placeholder="MRP Price" value="${mrp_price}" min="0" step="1" ${requiredAttr}/>
            </div>
            <div class="form-field">
        ...
        <div class="form-field variant-stock-field">
            ...
            <input type="number" class="variant-stock" name="variant-stock" placeholder="Stock" value="${stock}" min="0" step="1" ${requiredAttr}/>
        </div>
...
// --- START: CORRECTED toggleVariantForm ---
function toggleVariantForm(category) {
    ...
    // Determine if the product category uses variants
    const usesVariants = ['mobiles', 'seconds', 'laptops'].includes(category);
    ...
    if (variantFields && singlePriceField && pPriceInput && pMRPPriceInput && pStockInput && pDiscountEndDateInput && addVariantBtn) {
        if (usesVariants) {
            ...
            // Single product fields are NOT required
            pMRPPriceInput.required = false;
            pStockInput.required = false;

            addVariantBtn.style.display = 'inline-block';
            
            // Re-apply 'required' attribute to all current variant inputs
            $$('#variantsContainer .variant-group').forEach(group => {
                ...
                if (specNameInput) specNameInput.required = true;
                if (mrpPriceInput) mrpPriceInput.required = true;
                if (stockInput) stockInput.required = true;
            });

            if ($$('#variantsContainer .variant-group').length === 0) {
                // Ensures at least one variant row exists when switching to a variant category
                addVariantRow('', '', '', '', '', true);
            }
            ...
            // Single product fields are required
            pMRPPriceInput.required = true;
            pStockInput.required = true;

            addVariantBtn.style.display = 'none';
            
            // Remove 'required' attribute from all current variant inputs
            $$('#variantsContainer .variant-group').forEach(group => {
                const specNameInput = group.querySelector('.variant-name');
                const mrpPriceInput = group.querySelector('.variant-mrp-price');
                const stockInput = group.querySelector('.variant-stock');
                
                if (specNameInput) specNameInput.required = false;
                if (mrpPriceInput) mrpPriceInput.required = false;
                if (stockInput) stockInput.required = false;
            });
        }
        
        // Price field is always optional (can be null/blank for a discounted product)
        pPriceInput.required = false; 
    }
}
// --- END: CORRECTED toggleVariantForm ---
...
// Validation check: Check if required inputs are present and valid
if (specNameInput.required && !specNameInput.value.trim()) {
showMessage('Missing Specification', 'Please enter a specification name for all variants.');
specNameInput.focus();
return;
}
if (mrpPriceInput.required && (!mrpPriceInput.value || isNaN(parseFloat(mrpPriceInput.value)))) {
showMessage('Invalid MRP Price', 'Please enter a valid MRP price for all variants.');
mrpPriceInput.focus();
return;
}
if (stockInput.required && (!stockInput.value || isNaN(parseInt(stockInput.value)))) {
...
const variantMRPPrice = parseFloat(mrpPriceInput.value || 0);
const variantStock = parseInt(stockInput.value || 0);
